...
import com.okecode.parking_detector_plugin.service.ParkingDetectionService
...
class ParkingDetectorPlugin: FlutterPlugin, MethodCallHandler, EventChannel.StreamHandler, ActivityAware {
  ...
  override fun onAttachedToEngine(@NonNull flutterPluginBinding: FlutterPlugin.FlutterPluginBinding) {
    ...
    methodChannel = MethodChannel(flutterPluginBinding.binaryMessenger, "com.okecode.parking_detector_plugin/parking_detection")
    methodChannel.setMethodCallHandler(this)
    
    // Canal para eventos (eventos desde nativo a Flutter)
    eventChannel = EventChannel(flutterPluginBinding.binaryMessenger, "com.okecode.parking_detector_plugin/parking_events")
    eventChannel.setStreamHandler(this)

    // Emitir estado actual si existe (sticky) para suscriptores tardíos
    try {
      val current = com.okecode.parking_detector_plugin.service.ParkingDetectionService.getCurrentState()?.name
      if (current != null) {
